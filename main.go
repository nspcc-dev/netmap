...
func addSelect(c *ishell.Context) {
	...
	s := getState(c)
	s.ss = append(s.ss, netgraph.Select{
		Key:   c.Args[1],
		Count: int32(count),
	})
}
...
func addFilter(c *ishell.Context) {
	...
	op, ok := netgraph.Operation_value[c.Args[1]]
	if !ok {
		c.Err(errors.New("operation must be one of: EQ, NE, LT, LE, GT, GE"))
		return
	}
	s := getState(c)
	s.fs = append(s.fs, netgraph.Filter{
		Key: c.Args[0],
		F:   netgraph.NewFilter(netgraph.Operation(op), c.Args[2]),
	})
}
